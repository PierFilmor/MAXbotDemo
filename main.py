...
async def main():
    ...
    if webhook_url:
        ...
        # Для webhook нужно использовать FastAPI или другой сервер
        # Здесь пример для запуска через polling для локального тестирования
        logger.info("Для webhook запускайте: uvicorn main:app --host 0.0.0.0 --port 8080")
