...
def add_master(id: int, name: str, specialty: str):
    """Добавить мастера"""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def add_appointment(user_id: int, service_id: int, master_id: int, datetime_str: str, status: str) -> int:
    """Создать новую запись"""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            'INSERT INTO appointments (user_id, service_id, master_id, appointment_time, status) VALUES (?, ?, ?, ?, ?)',
            (user_id, service_id, master_id, datetime_str, status)
        )
        conn.commit()
